...
These tools must be installed in order to build Ariel Core from source.
...
### 4. Clone Ariel repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Ariel Core repository to a directory.
...
Ariel Core includes a GUI built with the cross-platform Qt Framework.
...
You can deploy a `.dmg` containing the Ariel Core application using `make deploy`.
...
There are many ways to configure Ariel Core, here are a few common examples:
...
Run the following in your terminal to compile Ariel Core:
...
Ariel Core should now be available at `./src/arield`.
...
``` bash
/Users/${USER}/Library/Application Support/Ariel/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Ariel"

touch "/Users/${USER}/Library/Application Support/Ariel/ariel.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Ariel/ariel.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Ariel/debug.log
```

## Other commands:

```shell
./src/arield -daemon      # Starts the ariel daemon.
