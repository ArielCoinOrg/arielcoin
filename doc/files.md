...
The data directory is the default location where the Ariel Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.ariel/`
macOS    | `$HOME/Library/Application Support/Ariel/`
Windows  | `%APPDATA%\Ariel\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `ariel.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Ariel blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `banlist.dat`         | Stores the addresses/subnets of banned nodes (deprecated). `arield` or `ariel-qt` no longer save the banlist to this file, but read it on startup if `banlist.json` is not present.
`./`               | `banlist.json`        | Stores the addresses/subnets of banned nodes.
`./`               | `ariel.conf`           | User-defined [configuration settings](bitcoin-conf.md) for `arield` or `ariel-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `arield.pid`           | Stores the process ID (PID) of `arield` or `ariel-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `arield` or `ariel-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [ariel.conf](bitcoin-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`ariel-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Ariel Core:
