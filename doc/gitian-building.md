Gitian building
================
*Setup instructions for a Gitian build of Ariel Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Ariel
...
- [Building Ariel Core](#building-ariel-core)
...
Clone the git repositories for Ariel and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/arielcoinorg/arielcoin --recursive
git clone https://github.com/arielcoinorg/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/arielcoin/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the Ariel repository under 'Fetch and create inputs' to install sources which require
...
To build Ariel Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Ariel repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ariel/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/arielcoinorg/arielcoin
...
For example:
```bash
URL=https://github.com/arielcoinorg/arielcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit ariel=${COMMIT} --url ariel=${URL} ../arielcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ariel=${COMMIT} --url ariel=${URL} ../arielcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ariel=${COMMIT} --url ariel=${URL} ../arielcoin/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-ariel:
./bin/gbuild --commit ariel=${COMMIT},cpp-eth-ariel=${ETHCOMMIT} --url ariel=${URL},cpp-eth-ariel=${ETHURL} ../ariel/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the ariel git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../arielcoinorg/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/arielcoinorg/ariel-detached-sigs.git

BTCPATH=/some/root/path/arielcoin
SIGPATH=/some/root/path/ariel-detached-sigs

./bin/gbuild --url ariel=${BTCPATH},signature=${SIGPATH} ../ariel/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ariel-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ariel-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/ariel-osx-build.assert
```
...
[arielcoinorg/gitian.sigs](https://github.com/arielcoinorg/gitian.sigs/) repository, or if that's not possible create a pull
request.
