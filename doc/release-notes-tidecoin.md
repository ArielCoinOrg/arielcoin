Arielcoin Core version 0.18.1 is now available from:

  <https://download.arielcoin.org/arielcoin-0.18.1/>
...
`/Applications/Arielcoin-Qt` (on Mac) or `arielcoind`/`arielcoin-qt` (on
Linux).
...
Arielcoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Arielcoin Core on unsupported systems.
...
10.10. Additionally, Arielcoin Core does not yet change appearance when
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@arielcoin.org) the Arielcoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 TDC),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in TDC per kilobyte, not litoshi per byte.
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Arielcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Arielcoin Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Arielcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Arielcoin Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart arielcoind with the `-deprecatedrpc=generate`
...
- A new `arielcoin-wallet` tool is now distributed alongside Arielcoin
...
This section describes planned changes to Arielcoin Core that may affect
other Arielcoin software and services.

- Since version 0.16.0, Arielcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Arielcoin Core 0.20 (expected about
  a year after 0.18), Arielcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Arielcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Arielcoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Arielcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Arielcoin Core
...
- A sub-project of Arielcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Arielcoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Arielcoin Core's own implementation, although entropy
  gathered by Arielcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Arielcoin Core a
...
- On macOS, Arielcoin Core now opts out of application CPU throttling
