Arielcoin Core version 0.13.2 is now available from:

  <https://download.arielcoin.org/arielcoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a arielcoin
...
ECDSA signatures inside Arielcoin transactions now use validation using
...
Previous versions of Arielcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Arielcoin Core was to raise the default minimum
relay fee.

Arielcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Arielcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running arielcoind without having to do any manual configuration.
...
Arielcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Arielcoin Core will relay transactions with
...
entirely in Arielcoin Core 0.13, and it is at this time undecided whether the
...
Arielcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Arielcoin Core automatically creates a hidden service to listen on, without
manual configuration. Arielcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Arielcoin Core is listening, and
...
Arielcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Arielcoin
Core to use floating fees. By default, Arielcoin Core will use floating
fees.
...
At all times, Arielcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) TDC.
Furthermore, Arielcoin Core will never create transactions paying less than
...
`arielcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /arielcoinrpc>
        ...
        # optional bypass arielcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< arielcoinrpc:<password>
    </Location>
...
Restarting arielcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in arielcoin.conf
...
arielcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/arielcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Arielcoin Core code base has
...
- `arielcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `arielcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Arielcoin scripting system that in combination with
...
Arielcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Arielcoin Core.
...
The two phases are distinct in the Arielcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Arielcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Arielcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Arielcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of arielcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of arielcoins being spent is exactly
...
- **Increased security for multisig:** Arielcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Arielcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Arielcoin's current ledger (called the UTXO set),
  ...
  note that Arielcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Arielcoin users to individually opt-in to almost any change in the Arielcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Arielcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Arielcoin
client to Arielcoin Core 0.13.2 to signal for segwit support. When a
...
Since Arielcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `arielcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `arielcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
