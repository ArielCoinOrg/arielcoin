...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateEmptyBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t nTime)
{
    ...
    pblock->hashStateRoot = uint256(h256Touint(dev::h256(globalState->rootHash())));
    pblock->hashUTXORoot = uint256(h256Touint(dev::h256(globalState->rootHashUTXO())));
//    std::cout << "BLOCK hashStateRoot BASED " << h256Touint(dev::h256(globalState->rootHash())).ToString() << std::endl;
...
public:
...

    {
        ...
        fEmergencyStaking = false; //gArgs.GetBoolArg("-emergencystaking", false);
        fAggressiveStaking = false; //gArgs.IsArgSet("-aggressive-staking");
