    ...
    m_results(1)
{
    ...
    ui->labelAmount->setToolTip(tr("The amount in ARIEL to send. Default = 0."));
    ui->labelSenderAddress->setToolTip(tr("The ariel address that will be used as sender."));
...
void SendToContract::setModel(WalletModel *_model)
{
    ...
    if (bCreateUnsigned) {
        ...
        ui->pushButtonSendToContract->setToolTip(tr("Creates a Partially Signed Ariel Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
    }
}
