...
static UniValue getkawpowhash(const JSONRPCRequest& request) {
    ...
    uint64_t nNonce;
    if (!ParseUInt6416(hex_nonce, &nNonce, 16))
        ...
        throw std::runtime_error(
            RPCHelpMan{"generatetoaddress",
                "\nMine blocks immediately to a specified address (before the RPC call returns)\n",
                {
                    ...
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated arielcoin to."},
            ...
            + "If you are running the arielcoin core wallet, you can get a new address to send the newly generated arielcoin to with:\n"
...
static UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Arielcoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Arielcoin is downloading blocks...");
