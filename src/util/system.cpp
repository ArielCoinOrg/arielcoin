...
const char * const BITCOIN_CONF_FILENAME = "arielcoin.conf";
...
#else
    const char* pszModule = "arielcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Arielcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Arielcoin";
#else
    // Unix
    return pathRet / ".arielcoin";
#endif
#endif
}
