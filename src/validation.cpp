...
#if defined(NDEBUG)
# error "Arielcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Arielcoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s ARL additional fees, %d delta bytes\n",
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    if (!CheckProofOfWork(block.GetHash(), block.nBits, consensusParams))
        return error("ReadBlockFromDisk: Errors in block header at %s hash: %s", pos.ToString(), block.GetHash().ToString());
...
void ThreadScriptCheck() {
    RenameThread("arielcoin-scriptch");
...
bool CChainState::AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        //std::cout << "hash: " << block.GetHash().ToString() << " pow:" << block.GetHashFull(mix).ToString() << std::endl;
        //std::cout.flush();
